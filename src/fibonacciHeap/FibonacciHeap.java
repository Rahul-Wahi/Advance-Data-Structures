...
//			return;
//		}
		//in fibonacci heap add at the last, because of doubly linked list
...
//		Node<T> minSibling = minPointer.sibling;
		...
		else
		{
			...
			//at this point we wont care about min value, min pointer will be updated to correct value after pairwise combine
	...
	Description: This function remove the given node from heap
	Return: Return min element value of Type T
	 *******************
	 */
	public T remove( Node<T> node )
	{
		...
		//This if will always run for childs ,because in top list, if there is only 1 node
	...
	Argument: Node<T> node, T newSmallValue
	Description: This function will update the node value if newSmallValue is smaller than stored value
	Return: None
	 *******************
	 */
	public void decreaseKey( Node<T> node , T newSmallValue )
	{
		...
		{
			System.out.print("Please provide smaller vale then the existing value " + node.data + " Provide : " + newSmallValue + " ") ;
			return ;
		}
	...
	/***********************
	Function Name: meld
	Argument: Node<T> tree1 (smaller data than tree2), Node<T> tree2
...
//		tree2.sibling = tree1Sibling;
	...
	Description: This function will pairwise combine given nodes and recursively combine the resulting node
